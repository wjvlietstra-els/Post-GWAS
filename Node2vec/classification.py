...
if writeOut:
    ...
    all_metrics = all_metrics[((all_metrics["variation"].isin(["normal", "combi"])) & (all_metrics["algorithm"] == "SVM") & (all_metrics["refset"] == "farashi")) |
                ((all_metrics["variation"].isin(["autoencode", "graphlet"])) & (all_metrics["algorithm"] == "LR") & (all_metrics["refset"] == "farashi")) |
                ((all_metrics["variation"].isin(["normal"])) & (all_metrics["algorithm"] == "KNN7") & (all_metrics["refset"] == "farashi p-value cutoff")) |
                ((all_metrics["variation"].isin(["autoencode"])) & (all_metrics["algorithm"] == "LR") & (all_metrics["refset"] == "farashi p-value cutoff")) |
                ((all_metrics["variation"].isin(["graphlet"])) & (all_metrics["algorithm"] == "KNN3") & (all_metrics["refset"] == "farashi p-value cutoff")) |
                ((all_metrics["variation"].isin(["combi"])) & (all_metrics["algorithm"] == "SVM") & (all_metrics["refset"] == "farashi p-value cutoff")) |
                ((all_metrics["variation"].isin(["normal"])) & (all_metrics["algorithm"] == "KNN9") & (all_metrics["refset"] == "DeRycke")) |
                ((all_metrics["variation"].isin(["autoencode"])) & (all_metrics["algorithm"] == "KNN3") & (all_metrics["refset"] == "DeRycke")) |
                ((all_metrics["variation"].isin(["graphlet", "combi"])) & (all_metrics["algorithm"] == "LR") & (all_metrics["refset"] == "DeRycke")) |
                ((all_metrics["variation"].isin(["normal", "graphlet"])) & (all_metrics["algorithm"] == "KNN1") & (all_metrics["refset"] == "Teslovich")) |
                ((all_metrics["variation"].isin(["autoencode", "combi"])) & (all_metrics["algorithm"] == "SVM") & (all_metrics["refset"] == "Teslovich"))
               ]
...
#     # In[30]:
    ...
    if len(outcomes2) > 0:
        ...
        if writeOut:
            ref_out = ref[["SNP ID", "nodeID", "predicted", "Class", "For-SNP rank"]].copy()
            ref_out["For-SNP rank"] = ref_out["For-SNP rank"].astype(int)
            ref_out.to_csv("/Users/vlietstraw/git/Post-GWAS/Node2vec/" + am_values["refset"] + " " + am_values["variation"] + " " + am_values["algorithm"] + " " + am_values["bp distance"] + " " + datetime.today().strftime("%d-%m-%Y") + ".csv", sep = ";", index = False)
...
all_metrics.to_csv("/Users/vlietstraw/git/Post-GWAS/Node2vec/all_variations_performance_metrics train-test separated " + datetime.today().strftime("%d-%m-%Y") + ".csv", sep = ";", decimal = ",", index = False)
